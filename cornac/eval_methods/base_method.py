...
def ranking_eval(
    ...
    for user_idx in tqdm(
        ...
        item_indices = np.nonzero(u_gt_pos_mask + u_gt_neg_mask)[
            0
        ]  # test set + non-interacted items(not val or test)

        u_gt_pos_items = np.nonzero(u_gt_pos_mask)[0]  # index of items of test set
