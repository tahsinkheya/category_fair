...
class Recommender:
    ...
    def dbg(self, name, t):
        if not torch.is_tensor(t):
            print(f"{name:32s} NOT A TENSOR ({type(t).__name__})")
            return
        fn = type(t.grad_fn).__name__ if t.grad_fn is not None else None
        print(
            f"{name:32s} dtype={str(t.dtype):13s} shape={str(tuple(t.shape)):16s} "
            f"req_grad={str(t.requires_grad):5s} grad_fn={fn}"
        )
    ...
    def differentiable_rank(self, user_idx, item_indices=None, k=-1, **kwargs):
        ...
        if k != -1:  # O(n + k log k), faster for small k which is usually the case
            # implementation 3 that better be differntiable
            s_k = torch.topk(item_scores, k, dim=-1).values[..., -1:]
            s_k = s_k.detach()

            w_soft = torch.sigmoid((item_scores - s_k) / 0.01)
            w_hard = (item_scores >= s_k).float()
            
            return w_hard + (w_soft - w_soft.detach()), item_scores, 0
            

        # return P,item_scores,0
