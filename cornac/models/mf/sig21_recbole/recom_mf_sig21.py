...
## part of this code is follwing the implementation  # @Author : Jiakai Tang
# @Email  : whut_tangjiakai@qq.com from recbole https://recbole.io/quickstart.html
...
from cornac.models.mf.sig21_recbole.layers import MLPLayers


class MF_Sig21(Recommender, ANNMixin):
    """Matrix Factorization with biases and adv training for learning fair feature representation. Reference:
Yunqi Li, Hanxiong Chen, Shuyuan Xu, Yingqiang Ge, and Yongfeng Zhang. 2021. Towards Personalized Fairness based on Causal Notion. Proceedings of the 44th International ACM SIGIR Conference on Research and Development in Information Retrieval.
    ...
    The implementation is only for filter module separate method for 1 sesitive attribute 
    ...
    * Koren, Y., Bell, R., & Volinsky, C. Matrix factorization techniques for recommender systems. \
    ...
    def __init__(
        ...
        self.dis_hidden_size_list = dis_hidden_size_list
        self.activation = activation
        self.sense_attr = [
            "gender"
        ]  # can be changed later to cater for more sensitive attribute
        self.filter_num, self.sst_dict = 1, {"gender": 1}
        self.sst_size = 2  # binary gender values
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.filter_layer = self.init_filter_layer()
    ...
    def init_filter_layer(self):
        # adding layer for filter modulw for gender
        f_layer = {}
        embedding_size = self.k
        for i in range(self.filter_num):
            filter_model = MLPLayers(
                [embedding_size, embedding_size * 2, embedding_size],
                activation=self.activation,
                bn=True,
                init_method="norm",
            )
            f_layer[i + 1] = filter_model.to(self.device)
    ...
    def init_dis_layer(self):
        ...
        if output_dim == 2:
            output_dim = 1
        dis_layer_dict[self.sst_attr[0]] = MLPLayers(
            [embedding_size] + dis_hidden_size_list + [output_dim],
            dropout=self.dis_dropout,
            activation=self.activation,
            bn=True,
            init_method="norm",
        ).to(self.device)
